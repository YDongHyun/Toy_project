...
pthread_t receive_tid[MAX_CLIENT_CNT];
...
void* receiveMsg(void* arg) {
  ...
  int tmp_idx=log_idx;
	char buf[MAX_CLIENT_CNT];
	while (!exitFlag[id]) {
    if(tmp_idx!=log_idx): 
		memset(buf, 0,MAX_CLIENT_CNT);
		 read(client_sock[id], buf, MAX_CLIENT_CNT);

		write(client_sock[id], chat_log[log_idx - 1], strlen(chat_log[log_idx - 1]));
		write(client_sock[id], "</br>", strlen("</br>"));
...
int main(int argc, char* argv[]) {
	...
	while (1) {
		...
		pthread_create(&receive_tid[id], NULL, receiveMsg, (void*)&id_table[id]);
		for (int i = 0; i < MAX_CLIENT_CNT; i++) {
			if (exitFlag[i] == 1) {
				exitFlag[i] = 0;
				pthread_join(tid[i], 0);
				pthread_join(receive_tid[i], 0);
				client_sock[i] = 0;
			}
		}
	}
